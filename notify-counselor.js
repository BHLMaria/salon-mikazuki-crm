...
export default async function handler(req, res) {
  ...
  // 入力の取得
  const {
    counselorEmail: counselorEmailRaw,
    counselorName,
    customerName,
    sessionDate,
    sessionNumber,
  ...
  // 公認心理師(1名・固定)。画面からの指定があれば優先、なければ初期値を使用
  // ※ 変更する場合は Vercel 環境変数 COUNSELOR_EMAIL で上書き可能
  const counselorEmail = counselorEmailRaw || process.env.COUNSELOR_EMAIL || 'cocoro.care.salon@gmail.com';
...
予防・養生支援を目的としたサービスです(医療行為ではありません)
  ...
  // Resend API経由で送信
  // ※ 本番運用: maria-beauty.com 認証済み。salon@maria-beauty.com から送信
  // ※ 変更する場合は Vercel 環境変数 RESEND_FROM_EMAIL で上書き可能
  const fromAddress = process.env.RESEND_FROM_EMAIL || 'SALON三日月 <salon@maria-beauty.com>';

  // 管理用の控え(CC): 予約実施のマネジメント用にサロン管理者へ同送
  // ※ 変更する場合は Vercel 環境変数 RESEND_CC_EMAIL で上書き可能
  const ccAddress = process.env.RESEND_CC_EMAIL || 'megumi.takahashi1971@gmail.com';
